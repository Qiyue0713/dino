...
def get_args_parser():
    ...
    # 选择保存 best 的指标（来自 evaluate() 返回的 test_stats keys）
    parser.add_argument("--best_metric", default="Top_1", type=str, help="Top_1 / Top_k")
...
def main(args):
    ...
    if not args.eval:
        ...
        # -------------------------- 新增：训练集随机截取8%样本 --------------------------
        # train_total_samples = len(dataset_train)
        # train_keep_samples = int(train_total_samples * 0.08)  # 计算8%样本数量
        # # 固定随机种子，确保每次训练采样结果一致（可复现）
        # random.seed(args.seed + utils.get_rank())  # 分布式训练下，每个进程种子略有差异，避免样本重复
        # # 随机抽取20%的样本索引
        # train_indices = random.sample(list(range(train_total_samples)), train_keep_samples)
        # # 构建训练集子集
        # dataset_train = torch.utils.data.Subset(dataset_train, train_indices)
        # logger.info(f"训练集已随机截取8%样本，原总数：{train_total_samples}，现总数：{len(dataset_train)}")
        # ----------------------------------------------------------------------------

    dataset_val = build_dataset(image_set='val', args=args, datasetinfo=dataset_meta["val"][0])
    # -------------------------- 新增：验证集随机截取8%样本 --------------------------
    # val_total_samples = len(dataset_val)
    # val_keep_samples = int(val_total_samples * 0.08)  # 计算8%样本数量
    # # 固定随机种子（与训练集区分，避免索引分布一致）
    # random.seed(args.seed + utils.get_rank() + 100)
    # # 随机抽取8%的样本索引
    # val_indices = random.sample(list(range(val_total_samples)), val_keep_samples)
    # # 构建验证集子集
    # dataset_val = torch.utils.data.Subset(dataset_val, val_indices)
    # logger.info(f"验证集已随机截取8%样本，原总数：{val_total_samples}，现总数：{len(dataset_val)}")
    # ----------------------------------------------------------------------------
    ...
    if args.eval:
        ...
        if utils.is_main_process():
            top1 = test_stats.get("Top_1", None)
            topk = test_stats.get("Top_k", None)

            msg = []
            if top1 is not None: msg.append(f"Top_1={top1:.4f}")
            if topk is not None: msg.append(f"Top_k={topk:.4f}")
            print("[Eval] " + " | ".join(msg))
    ...
    """
    修改地方：best 保存按 best_metric（默认 Acc@1），不再用 mAP holder
    """
    # best_map_holder = BestMetricHolder(use_ema=False)
    best_holder = BestMetricHolder(init_res=0.0, better="large", use_ema=False)
    metric_key = getattr(args, "best_metric", "Top_1")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # >>> MOD-8: best 判断基于 metric_key（Acc@1/Acc@5/Top1/Top5）
        if metric_key not in test_stats:
            raise KeyError(f"best_metric='{metric_key}' not found in test_stats keys: {list(test_stats.keys())}")

        cur_metric = float(test_stats[metric_key])
        is_best = best_holder.update(cur_metric, epoch, is_ema=False)
