...
class ODVGDataset(VisionDataset):
    ...
    def __getitem__(self, index: int):
        ...
        # target = {}
        # target["size"] = torch.as_tensor([int(h), int(w)])
        # target["orig_size"] = torch.as_tensor([int(h), int(w)])  # 评测用：原图大小
        # # 优先用标注里自带的 image_id，否则用 index
        # target["image_id"] = torch.as_tensor(int(meta.get("image_id", index)))
        #
        # target["cap_list"] = caption_list
        # target["caption"] = caption
        #
        # # 训练/增强会修改 target["boxes"]/["labels"]，因此保留一份原始 GT 供评测用
        # target["boxes"] = boxes
        # target["labels"] = classes
        # target["boxes_orig"] = boxes.clone()
        # target["labels_orig"] = classes.clone()
